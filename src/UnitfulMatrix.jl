...
@inline function DimensionalData.rebuildsliced(f::Function, A::AbstractUnitfulVecOrMat, data::AbstractArray, I::Tuple; exact= exact(A))
    ...
    if (udomain isa Unitful.Unitlike || urange isa Unitful.Unitlike )
...
function slicedmatrix(urange,udomain) 
    unt = Array{Unitful.Unitlike}(undef,length(urange),length(udomain))
...
function UnitfulMatrix(A::AbstractMatrix)
    numbers = ustrip.(A)
    M,N = size(numbers)
    unitdomain = Vector{Unitful.Unitlike}(undef,N)
    unitrange = Vector{Unitful.Unitlike}(undef,M)
